...
-- Sşstem bilgileri
--PerformHttpRequest(Config.SistemLogu, function(Error, Content, Head) end, 'POST', json.encode({username = SystemName, content = '**Kasırga Roleplay** LOG SISTEMI AKTIF'}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('chatMessage', function(Source, Name, Message)
	...
	if not IsCommand(MessageSplitted, 'Blacklisted') then
		...
		if GetIDFromSource('steam', Source) then
			PerformHttpRequest('http://steamcommunity.com/profiles/' .. tonumber(GetIDFromSource('steam', Source), 16) .. '/?xml=1', function(Error, Content, Head)
				...
				for i, Line in ipairs(SteamProfileSplitted) do
					if Line:find('<avatarFull>') then
						...
						TriggerEvent('brkblnt:ToDiscord', Webhook, Name .. ' [ID: ' .. Source .. ']', Message, AvatarURL, false, Source, TTS) --Discord mesajı
						break
					end
				end
			end)
			...
			TriggerEvent('brkblnt:ToDiscord', Webhook, Name .. ' [ID: ' .. Source .. ']', Message, AvatarURL, false, Source, TTS) --Discorda mesaj gönderme
		end
	end
end)

--Event to actually send Messages to Discord
RegisterServerEvent('brkblnt:ToDiscord')
AddEventHandler('brkblnt:ToDiscord', function(WebHook, Name, Message, Image, External, Source, TTS)
